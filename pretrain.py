...
def run(data, args):
    ...
    optimizer = torch.optim.Adam([
            dict(params=model.parameters(), weight_decay=0)], lr=0.01)
    ...
    for count in pbar:
        ...
        for epoch in range(0, args.epochs):
            ...
            for epoch_c in range(0, args.c_epochs):
                ...
                # h = encoder(data.x + args.delta * model(data.x) / deg_norm.unsqueeze(-1) , data.edge_index, data.adj_norm_sp)
